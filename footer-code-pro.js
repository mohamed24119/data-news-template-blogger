	...
	for(let i = 0; i < $parent_element.length; i++) {
		...
		if(data_posts_count > 0) {
			function fun_fetch_posts(){
                ...
                if($parent_element[i].hasAttribute("data-blog-id")) {data_blog_id = $parent_element[i].getAttribute("data-blog-id");} 
				...
				if($parent_element[i].classList.contains("last-posts")) {
				    if($parent_element[i].hasAttribute("data-random-posts") && $parent_element[i].getAttribute("data-random-posts") == "true") {
                        ...
                        $api_blog = `${APIBLOGGER}${data_blog_id}/posts?key=${api_key_google}&maxResults=${data_max_results}${fields_blogposts}`;
                        fetch($api_blog)
                        ...
                        $api_blog = `${APIBLOGGER}${data_blog_id}/posts?key=${api_key_google}&maxResults=${data_posts_count}${fields_blogposts}`;
                        fetch($api_blog)
